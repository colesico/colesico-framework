...
public class AuthenticationManagerImpl implements AuthenticationManager {
    ...
    private Authenticator<AuthenticationRequest> authenticator(Class<? extends AuthenticationRequest> contextClass) {
    ...
    public AuthenticationResult login(AuthenticationRequest request) {
        return authenticator(request.getClass()).login(request);
    }

    @Override
    public void logout(Identity<?> identity) {
        @SuppressWarnings("unchecked")
        Class<? extends AuthenticationRequest> contextClass =
                (Class<? extends AuthenticationRequest>) identity.claims().get(Identity.AUTHENTICATOR_CLAIM);
