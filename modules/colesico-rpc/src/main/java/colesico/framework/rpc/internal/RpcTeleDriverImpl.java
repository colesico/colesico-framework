...
public class RpcTeleDriverImpl implements RpcTeleDriver {
    ...
    @Inject
    public RpcTeleDriverImpl(TRWFactory ioc, RpcExchange exchange, ThreadScope threadScope) {
        this.trwFactory = ioc;
    ...
    public <T> void invoke(T service, MethodInvoker<T, RpcDataPort> invoker, RpcTIContext invCtx) {
        ...
        RpcDataPort dataPort = new RpcDataPortImpl(trwFactory, request, response);
